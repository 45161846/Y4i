...
fun CustomOverflowText(
    ...
    var inProgress by remember(l, r) {
        mutableStateOf(l < r)
    }

    val shortText by remember(mid) {
        mutableStateOf(
            if (mid < text.length - overflowSuffix.length) text.subSequence(0 until mid)
                .toString() + overflowSuffix
            else text
        )
    }
    ...
    Text(
        ...
        style = style.copy(
            ...
            color = if (inProgress) Color.Transparent else style.color
        ),
        modifier = modifier
            .animateContentSize()
