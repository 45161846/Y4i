package com.example.russian.main.theme

import androidx.compose.ui.graphics.Color

val Black = Color(0xFF000000)
val Black11 = Color(0xFF111111)
val LightBlack = Color(0xFF222222)
val DarkGrey = Color(0xFF282828)
val LightGrey69 = Color(0xFF696969)
val LightGreyBB = Color(0xFFBBBBBB)
val WhiteDD = Color(0xFFDDDDDD)
val WhiteEE = Color(0xFFEEEEEE)
val AbsoluteWhite = Color(0xFFFFFFFF)

val TransparentBlack = Color(0x30000000)
val TransparentWhite = Color(0x45FFFFFF)
val TransparentWhite80 = Color(0x803D7BF1)

val Red = Color(0xFFD72929)
val Green = Color(0xFF1FB736)

val LightModeAnswerCorrect = Color(0xFF25CB00)
val LightModeAnswerIncorrect = Color(0xFFD50000)

val DarkModeAnswerCorrect = Color(0xFF4FF34A)
val DarkModeAnswerIncorrect = Color(0xFFFD4949)

val LightRed = Color(0xFFF47272)
val LightGreen = Color(0xFF76F472)

val LightBlue = Color(0xFF90BBE1)
val LightBlueContainer = Color(0xFF485D70)
val WhiteBlue = Color(0xFFBFD5F8)
val BlackBlue = Color(0xFF3D4856)

val WhitePurple = Color(0xFF94A3B8)
val BlackPurple = Color(0xFF475569)

val SaturatedYellow = Color(0xFFE0B342)